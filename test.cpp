...
//Network Table setup
    ...
    VideoCapture cap(0);
    VideoCapture cap2("http://roboRIO-3482-frc.local:1182/?action=stream");
...
//    namedWindow("HSVThresholded", CV_WINDOW_AUTOSIZE);
	...
	imshow("dialted", thresholded2);

	

	findContours(thresholded2, contours2, RETR_EXTERNAL, CHAIN_APPROX_SIMPLE);
	...
	double filterContoursMinArea = 250.0;  // default Double
        ...
        double filterContoursMinVertices = 15.0;  // default Double
        double filterContoursMinRatio = 2.0;  // default Double
        double filterContoursMaxRatio = 25.0;  // default Double
